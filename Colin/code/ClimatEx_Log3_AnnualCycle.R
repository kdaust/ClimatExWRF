...
# Plots showing the annual and monthly ClimatEx WRF climatology maps of tmin, tmax, and pr. 
...
for(color in colors){
  ...
  for(projection in projections){
    ...
    for(e in 1:3){
      ...
      X <- if(e == 3) wrfclimatex[[13]]/monthdays[13] else wrfclimatex[[13]]
      X <- crop(X, studyarea)
      if(e==3){
        values(X) <- log2(values(X))
        values(X)[!is.finite(values(X))] <- NA
      ...
      lim1 <- quantile(if(e == 3) prlim else values(wrfclimatex), top, na.rm=T)
      values(X)[which(values(X)>lim1)] <- lim1
      bottom <- if(e == 3) 0 else 0.005
      lim0 <- quantile(if(e == 3) prlim else values(wrfclimatex), bottom, na.rm=T)
      ...
      for(m in 1:12){
        ...
        X <- if(e == 3) wrfclimatex[[m]]/monthdays[m] else wrfclimatex[[m]]
        X <- crop(X, studyarea)
        if(e==3){
          values(X) <- log2(values(X))
          values(X)[!is.finite(values(X))] <- NA
